  ...
  useEffect(() => {
    const fetchContracts = async () => {
      ...
      try {
        ...
        const contractsResponse = await api.get(`/api/contracts/freelancer/${freelancerId}`);
  ...
  return (
    <section className="space-y-6">
      <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
        <h2 className="text-2xl font-semibold text-slate-900">My Contracts</h2>
        <p className="mt-2 text-sm text-slate-600">Track all contracts assigned to you.</p>
      </div>
